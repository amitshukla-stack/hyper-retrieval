"""
Stage 1 v2 — Extract symbols, bodies, call graph, log patterns, and edges.
Supports: Haskell (.hs), Rust (.rs), Python (.py), Groovy (.groovy/.grails)
Output:  pipeline/output/raw_graph.json
         pipeline/output/body_store.json      (fn_id → body text)
         pipeline/output/call_graph.json      (fn_id → {callees, callers})
         pipeline/output/log_patterns.json    (fn_id → [log patterns])
"""
